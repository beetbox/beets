...
For instance you can create plugins that add new commands to the command-line interface,
listen for events in the beets lifecycle or extend the autotagger with new metadata sources.
...
The meat of your plugin goes in ``myawesomeplugin.py``. Every plugin has to extend the
:class:`beets.plugins.BeetsPlugin` abstract base class [#baseclass]_. For instance,
a minimal plugin without any functionality would look like this:
