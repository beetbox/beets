...
For multi-valued tags (such as ``artists``, ``albumartists``, ``remixers``, or
``lyricists``), a regular expression search must be used to search for a single
value within the multi-valued tag.
