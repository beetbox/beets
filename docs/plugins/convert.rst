...
instead. Note that configuration options such as ``embed`` that modify the
output files after the transcoding step will cause the original files to be
modified as well if ``--link`` is enabled. For this reason, it is highly
recommended not use to ``--link`` and ``embed`` at the same time.
