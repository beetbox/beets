...
- **synced**: Prefer synced lyrics over plain lyrics if a source offers them.
  ...
  plugin stores the timestamped data in the ``SYLT`` (synchronized lyrics)
  frame and plain text (without timestamps) in the ``USLT`` (unsynchronized
  lyrics) frame, so players that support only one of the two formats can still
  show the correct lyrics.
