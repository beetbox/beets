...
* **The library.** The beets *Library* is a database that catalogs your music.
  The Library does not contain any music files itself;
because it's a database, it saves the paths (i.e., the filenames)
  to your music files. The Library is composed of *Items* (songs) and *Albums*.
  The Album is a collection of Items, and it also contains its own, separate
  album-level metadata that is shared among all of its tracks. Some Items belongs
  to an Album, and others don't: those are called *singletons*.
  Everything that beets does interacts with the Library and the Items and
  Albums it contains.

* **Queries.** In beets, you use *queries* to select Albums and Items.
  Nearly every beets command takes a query as
...
* **Paths and destinations.** Each Item has a *destination*, which is the path where
  ...
  The destination consists of two parts: *root directory* (specified as :ref:`directory`
  in the configuration) and the fragment, which is generated based on your path configurations (see :ref:`path-format-config`).
  You specify the rules for generating the fragment using queries and
  templates, and when beets wants to know where a file "belongs," it uses them
  to compute a destination.
  The :ref:`move-cmd` command copies or moves files to their destinations.
