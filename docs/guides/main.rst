...
* **Data Model**. The beets *Library* is a database that catalogs your music, which 
  ...
  album-level metadata that is shared among all of its tracks. Each *Item* belongs
...
* **Paths and Destinations**. Each *Item* has a *destination*, which is the path where
  ...
  based on the path format templates specified by the user. The user specifies 
  query/template pairs in the config file; the template corresponding to the first 
  query to match a given Item is used to generate the Item's destination. The 
  ``beet move`` command works by copying/moving files to a path generated by 
  concatenating a different root directory with each Item's destination fragment.
  For more information please see `Path Configuration`_.
