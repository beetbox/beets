...
beets`` if you run into permissions problems). If you have pip but wish to update (uninstall and install the current version), run ``python -m pip install –upgrade pip``. Ensure the directory with pip’s scripts are added to ``PATH``.
...
2. If you haven't done so already, set your ``PATH`` environment variable to
   include Python and its scripts. To do so, press Win + R to open the Run dialog box. Next, type ``sysdm.cpl`` to open "System Properties". After, under the  “Advanced” tab, click the “Environment Variables” button, and look for the ``PATH`` variable in the table. Add the following to the end of the variable's value: 
...
4. You're all set! Type ``beet`` at the command prompt to make sure everything's
   in order. (If pip’s scripts are not in ``PATH``, alternatively run the beets package from the command line by typing ``python3 -m beets``.)
