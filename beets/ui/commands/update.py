...
def _resolve_update_fields(fields, exclude_fields, move):
    if move and fields is not None and "path" not in fields:
        # Keep the database path in sync when files are moved.
        fields.append("path")

    if fields is None:
        # No explicit fields were provided, so update all media fields.
    ...
    # Album fields follow the same selection rules as item fields.
    ...
    return item_fields, album_fields


def _update_single_item(lib, item, move, pretend, item_fields, affected_albums):
    # Item deleted?
    ...
    # Special-case album artist when it matches track artist.
    ...
    if not pretend:
        if changed:
            ...
            # The file's mtime was different, but there were no metadata
            # changes. Store the new mtime so we don't re-check forever.
            item.store(fields=item_fields)


def _update_affected_album(lib, album_id, move, item_fields, album_fields):
...
def update_items(lib, query, album, move, pretend, fields, exclude_fields=None):
    ...
    with lib.transaction():
        ...
        item_fields, album_fields = _resolve_update_fields(
            fields, exclude_fields, move
        )
        affected_albums = set()
        for item in items:
            _update_single_item(
                lib, item, move, pretend, item_fields, affected_albums
            )
        ...
        for album_id in affected_albums:
            _update_affected_album(
                lib, album_id, move, item_fields, album_fields
            )
